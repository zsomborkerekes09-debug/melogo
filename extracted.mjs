        ...
        import { getAuth, onAuthStateChanged, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, GoogleAuthProvider, OAuthProvider, signInWithRedirect, getRedirectResult, sendPasswordResetEmail } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";
        ...
        getRedirectResult(auth).then(async (result) => {
            if (result) {
                ...
                console.log("Sikeres redirect bejelentkezés:", user.email);
                ...
                try {
                    ...
                    if (!userSnap.exists()) {
                        ...
                        await updateDoc(userRef, { lastLogin: serverTimestamp(), photoURL: user.photoURL || userSnap.data().photoURL || '' });
                        const data = userSnap.data();
                        if (data.name) userName = data.name;
                        role = data.defaultRole || data.role || 'worker';
                    }
                    localStorage.removeItem('melogo_pending_role');
                    window.doEnterApp(userName, user.email, role);
                } catch (e) {
                    console.error("Hiba redirect feldolgozása közben:", e);
                }
            }
        }).catch(error => {
        ...
        window.setupFirestoreListeners = function(role) {
            ...
            function mergeFirestoreChats(newChats, roleType) {
                newChats.forEach(c => { c.isFirestore = true; c.roleType = roleType; });
        ...
        // Window.saveToCloud removed to avoid batch overwrites of Firestore collections
        ...
        async function loginWithApple() {
            ...
            try {
                ...
                await window.firebaseAPI.signInWithRedirect(window.firebaseAuth, provider);
            } catch (error) {
 ...
 updateAvatars();
        ...
        async function loginWithGoogle() {
            if (!window.firebaseAPI || !window.firebaseAPI.signInWithRedirect) {
...
function filterWorkerByExactJob(jobName) {
        ...
        async function workerApplyToJob() {
            ...
            setTimeout(() => {
                // Keep worker in their current view
            }, 500);
        ...
        function confirmCloseJob(event) {
            ...
            if (chat) {
                ...
                if (chat.isFirestore && window.firebaseAPI && window.firebaseDb) {
                    window.firebaseAPI.updateDoc(
                        window.firebaseAPI.doc(window.firebaseDb, "chats", chat.id),
                        { status: 'completed', lastMsg: 'A munka sikeresen lezárva' }
