        ...
        async function deleteAccount() {
            const confirmed = confirm("Biztosan véglegesen törölni szeretnéd a fiókodat? Ez a művelet nem visszavonható!");
            if (confirmed) {
                if (window.firebaseAuth && window.firebaseAuth.currentUser) {
                    ...
                    try {
                        ...
                        alert("Fiókod sikeresen törlésre került.");
                        closeLogoutConfirm();
                        logoutApp();
...
let settingsConfirmedAddress = null;
        ...
        async function saveSettings() {
            ...
            let phoneVal = document.getElementById('settings-phone')?.value || '';
        ...
        function setGPSStatus(active, cityName) {
            if (gameState.gpsActive === active && gameState.locationMode === 'gps') {
                // If already active and in GPS mode, just update distances
                recalculateJobDistances();
        ...
        async function loadChatHeaderAndStatus(chatId, chatData) {
            ...
            const partnerName = partnerData ? (partnerData.name || partnerData.email) : (!isEmployerChat ? chatData.employerName : chatData.workerName);
            const nameEl = document.getElementById('chat-header-partner-name');
            if (nameEl) {
                nameEl.innerText = partnerName || 'Névtelen partner';
            }
            
            const avatarImg = document.getElementById('chat-header-partner-avatar');
            const avatarPlaceholder = document.getElementById('chat-header-avatar-placeholder');
            ...
            // Task Name
            const taskEl = document.getElementById('chat-header-task-text');
            if (taskEl) {
                taskEl.innerText = jobTitle || 'Munka';
            }
            
            // Address
            const addrEl = document.getElementById('chat-header-address-text');
            if (addrEl) {
                addrEl.innerText = jobLocation || 'Nincs megadva';
            }
            
            const addrRow = document.getElementById('chat-header-row-address');
            if (addrRow) {
                addrRow.onclick = function() {
                    if (jobLocation) {
            ...
            const timeEl = document.getElementById('chat-header-time-text');
            if (timeEl) {
                timeEl.innerText = jobTimeStr || 'Nincs megadva';
            }
            
            // Phone (Visible to both parties after acceptance)
            const phoneRow = document.getElementById('chat-header-row-phone');
            const phoneEl = document.getElementById('chat-header-phone-text');
