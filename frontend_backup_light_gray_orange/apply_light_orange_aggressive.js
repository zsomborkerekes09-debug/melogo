...
const aggressiveLightCSS = `
<style id="aggressive-light">
    :root, body, #app-container, .apple-premium-dark-mode {
...
if (content.includes('<style id="light-orange-glow">')) {
    content = content.replace(/<style id="light-orange-glow">[\s\S]*?<\/style>/, '');
}
if (content.includes('<style id="aggressive-light">')) {
    content = content.replace(/<style id="aggressive-light">[\s\S]*?<\/style>/, '');
}

content = content.replace(/<\/head>/, aggressiveLightCSS);

fs.writeFileSync(file, content, 'utf8');
console.log('Aggressive Light Mode applied!');
