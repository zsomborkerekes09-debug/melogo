import { F as Firestore, r as FirestoreError, ax as V, a7 as documentId$1, i as __PRIVATE_fieldPathFromArgument, ay as __PRIVATE_hardAssert, az as __PRIVATE_parseData, aA as __PRIVATE_isString, aB as __PRIVATE_toStringValue, aC as __PRIVATE_isNumber, X as VectorValue, av as vector, aD as __PRIVATE_isPlainObject, aE as __PRIVATE_isCollectionReference, G as DocumentReference, s as d, E as CollectionReference, e as __PRIVATE_getDatastore, d as __PRIVATE_cast, j as __PRIVATE_newUserDataReader, h as __PRIVATE_LiteUserDataWriter, aF as __PRIVATE_invokeExecutePipeline, O as ObjectValue, aG as FieldPath$1, f as __PRIVATE_mapToArray, aH as __PRIVATE_toMapValue, aI as __PRIVATE_isCollectionGroupQuery, aJ as __PRIVATE_isDocumentQuery, a6 as doc, aK as __PRIVATE_queryNormalizedOrderBy, u as fail, aL as toNumber, aM as __PRIVATE_toPipelineValue, aN as __PRIVATE_isUserData, aO as FieldFilter, aP as CompositeFilter } from './common-8e1d9482.esm.js';
...
class PipelineSource {
    ...
    _validateReference(e) {
        ...
        if (!t.isEqual(this.databaseId)) throw new FirestoreError(d.INVALID_ARGUMENT, `Invalid ${e instanceof CollectionReference ? "CollectionReference" : "DocumentReference"}. The project ID ("${t.projectId}") or the database ("${t.database}") does not match the project ID ("${this.databaseId.projectId}") and database ("${this.databaseId.database}") of the target database of this Pipeline.`);
    }
}
 ...
 */ function execute(e) {
    if (!e._db) return Promise.reject(new FirestoreError(d.FAILED_PRECONDITION, "This pipeline was created without a database (e.g., as a subcollection pipeline) and cannot be executed directly. It can only be used as part of another pipeline."));
    const t = __PRIVATE_getDatastore(e._db), n = __PRIVATE_cast(e._db, Firestore), r = __PRIVATE_newUserDataReader(n).ce(3 /* UserDataSource.Argument */ , "execute");
    e._readUserData(r);
