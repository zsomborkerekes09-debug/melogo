...
`        #worker-slider { width: 400%; }
        #worker-slider .screen { width: 25%; }
        #employer-slider { width: 300%; }
        #employer-slider .screen { width: 33.333%; }`,
`        #app-slider { width: 400%; }
        #app-slider .screen { width: 25%; }
        ...
        #home-employer-view { display: none; flex-direction: column; }
        #home-worker-view { display: flex; flex-direction: column; }`);
...
`        <!-- ======================================================= -->
        ...
        <!-- ======================================================= -->
        <div class="phone-column" style="margin: 0 auto;">
...
const oldLoginStart = `                <!-- DIÁK ONBOARDING / ELSŐ REGISZTRÁCIÓ SCREEN -->`;
const oldLoginEnd = `                <!-- Slider -->
                <div class="screen-slider" id="worker-slider">`;

const newLoginBlock = `                <!-- UNIFIED ONBOARDING SCREEN -->
...
const startIdxLogin = content.indexOf(oldLoginStart);
const endIdxLogin = content.indexOf(oldLoginEnd);

if (startIdxLogin !== -1 && endIdxLogin !== -1) {
    content = content.substring(0, startIdxLogin) + newLoginBlock + content.substring(endIdxLogin + oldLoginEnd.length);
}
                    ...
                    <div class="screen" id="home-screen-scroll-container">
                        <div class="header-midnight">
                            <div class="header-top" style="justify-content: center; margin-bottom: 12px;">
                                <div class="brand-logo" style="font-size: 20px;">Melo<span>Go</span></div>
                            </div>
                        ...
                        <div id="home-employer-view">
                            <div style="padding: 20px;">
                                <button class="btn" onclick="openEmployerFormOverlay()" style="background-color: var(--color-blue-uber); box-shadow: 0 4px 15px rgba(0, 122, 255, 0.2);">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" style="margin-right: 4px;"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
                                    Adj fel új munkát
                                </button>
...
content = content.replace(/onclick="navigateWorker\(/g, 'onclick="navigateApp(');

// 7. Update bottom nav - make it a single nav for #phone-app and delete the employer phone completely.
const employerPhoneStart = `        <!-- ======================================================= -->
        <!-- 2. JOBB OLDALI TELEFON: MUNKÁLTATÓ (HIRDETŐ) NÉZET -->`;
const employerPhoneEnd = `    <!-- TÁJÉKOZTATÓ SZEKCIÓ ASZTALI NÉZETRE -->`;

const startIdxEmp = content.indexOf(employerPhoneStart);
const endIdxEmp = content.indexOf(employerPhoneEnd);

if (startIdxEmp !== -1 && endIdxEmp !== -1) {
    content = content.substring(0, startIdxEmp) + content.substring(endIdxEmp);
}

// Extract the "Employer Job Posting Form" (Screen 1 of employer) into an overlay
const navStart = `                <!-- Alsó navigáció (4 GOMBOS DIÁK NAVIGÁCIÓ) -->`;
const navReplace = `                <!-- Új munka feladása Overlay -->
...
content = content.replace(navStart, navReplace);
...
content = content.replace('id="worker-msg-badge"', 'id="app-msg-badge"');
...
const jsBlock1New = `        // ==========================================
        ...
        function logoutApp() {
            ...
            localStorage.removeItem('melogo_active_role');
            location.reload();
        }
...
content = content.replace(jsBlock1, jsBlock1New);

// 9. Update navigateWorker to navigateApp
const navJs = `        // 1. DIÁK NAVIGÁCIÓ
...
const navJsNew = `        // EGYESÍTETT NAVIGÁCIÓ
...
content = content.replace(navJs, navJsNew);

// Delete old employer navigation
content = content.replace(/ \/\/ 2\. HIRDETŐ NAVIGÁCIÓ[\s\S]*?function openEmployerJobDetail/m, "function openEmployerJobDetail");

// Delete old login functions
content = content.replace(/function loginWorker\(\)[\s\S]*?function filterWorkerJobs/m, "function filterWorkerJobs");

content = content.replace("function logoutWorker() {", "function logoutWorker() { logoutApp();");
content = content.replace("function logoutEmployer() {", "function logoutEmployer() { logoutApp();");

// Update publish job to close overlay
content = content.replace("function employerPublishJob() {", `function employerPublishJob() {
            closeEmployerFormOverlay();`);

fs.writeFileSync(filepath, content, 'utf8');
console.log('Refactor complete using Node.js');
