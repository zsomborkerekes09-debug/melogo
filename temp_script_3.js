...
let settingsConfirmedAddress = null;
        ...
        async function saveSettings() {
            ...
            const birthYearVal = document.getElementById('settings-birthyear')?.value ? parseInt(document.getElementById('settings-birthyear').value.trim()) : null;
            ...
            let phoneVal = document.getElementById('settings-phone')?.value.trim() || '';
        ...
        function getFilteredAndSortedJobs() {
            ...
            let jobs = mockJobs.filter(j => {
                ...
                return j.distance === null || j.distance === undefined || j.distance <= activeRadius;
            });
        ...
        function renderDateFilter() {
            ...
            for (let i = 0; i < 14; i++) {
                ...
                html += `
                    <div onclick="setDateFilter('${isoDate}')" style="min-width: 52px; height: 64px; border-radius: 12px; border: 1px solid ${isActive ? 'var(--color-text)' : '#E5E7EB'}; background: ${isActive ? 'var(--color-text)' : '#fff'}; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; flex-shrink: 0; box-sizing: border-box;">
        ...
        window.executeEmployerAccept = async function(chatId) {
            ...
            try {
                ...
                try {
                    ...
                    const myUid = window.firebaseAuth?.currentUser?.uid || 'N/A';
